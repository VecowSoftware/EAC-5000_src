...
static int cam_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if(ctrl->id == V4L2_CID_HDR || ctrl->id == V4L2_CID_FRAME_SYNC){
		...
		if( priv->change_sync == 1 && !is_sync_changed && priv->got_pwm_handle){
			dev_info(&client->dev," Recalibrating PWM For HDR mode \n");
			pwm_disable(priv->pwm_dev);
			pwm_config(priv->pwm_dev, PWM_28HZ_DUTY, PWM_28HZ_PERIOD);
			pwm_enable(priv->pwm_dev);
			is_sync_changed = 1;
			priv->change_sync = 0;
		}
		else if( cam_track == num_cam && is_sync_changed && priv->got_pwm_handle){
			is_sync_changed = 0;
...
skip:
	...
	deser_enable = of_get_named_gpio(node, "deser-reset-gpios", 0);
	if(deser_enable > 0) {
		debug_printk("deser_enable = %d \n", deser_enable);		
		err = gpio_request(deser_enable,"deser-reset-gpios");
		if (err < 0) {
